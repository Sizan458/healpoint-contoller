...
const Dashboard = () => {
  const{ getDashboardData, dashboard, aToken, cancelAppointment}=useContext(AdminContext)
  ...
  return dashboard && (
    <div className="m-5"> 
       <div className="flex flex-wrap gap-3 ">
        ...
        <div  className="flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all duration-300">
          <img src={assets.appointments_icon} alt="appointments icon"  className="w-14 h-14"/>
          <div>
       ...
       <div className="bg-white">
        ...
        <div className="pt-4 border border-t-0">
         {
          dashboard.latestAppointments && dashboard.latestAppointments.map((item, index)=>(
           <div key={index} className="flex item-center px-6 py-3 gap-3 hover:bg-gray-100">
            <img src={item.docData.image} alt={item.docData.name}className="rounded-full w-10 h-10"/>
